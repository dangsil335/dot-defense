...
둘 중 하나라도 실패하면 **아무것도 표시하지 않는다**(빈 카드).
...
| 상황 | 처리 |
|---|---|
| 대표 미설정 | **빈 카드**(일러 없음). 그 판에 쓴 캐릭으로 폴백하지 않는다 — 설정해야 뜬다는 동기를 만들기 위함 |
| 대표로 건 스킨이 레지스트리에서 삭제됨 | 미설정과 동일 취급 → 빈 카드. 픽커에서도 선택 해제로 표시 |
| 대표 일러 해제 | 픽커 맨 앞 `✕` 항목으로 언제든 미설정으로 되돌릴 수 있다 |
