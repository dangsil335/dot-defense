...
### 1-5b. `draw(ab)` — **능력마다 전용 그리기. 공용 헬퍼 하나로 때우지 말 것**

이 항목이 없어서 v784~788 의 플린타·알바가 **전용 그리기 0줄**로 나갔다.
기존 21명은 능력마다 31~227줄의 전용 `draw` 를 갖는데, 신규 둘만 헬퍼 하나를
색만 바꿔 4종이 공유해서 **로스터에서 가장 개성 없는 캐릭이 됐다.**
...
i=h.index('const ABILITY_DEFS'); d=0;st=False;j=i
while j<len(h):
    if h[j]=='{': d+=1;st=True
    elif h[j]=='}':
        d-=1
    ...
    j+=1
b=h[i:j+1]
ms=list(re.finditer(r'^    (\w+):\s*\{',b,re.M))
D={m.group(1):b[m.start():(ms[k+1].start() if k+1<len(ms) else len(b))] for k,m in enumerate(ms)}
sg=h.index('CHAR_SIG_ABILITIES')
S=dict(re.findall(r'^    (\w+):\s*\[([^\]]*)\],',h[sg:sg+3000],re.M))
def db(x):
    k=x.find('draw(ab)')
    if k<0: return ''
    dd=0;s2=False;e=k
    while e<len(x):
        if x[e]=='{': dd+=1;s2=True
            ...
            dd-=1
            if s2 and dd==0: break
        e+=1
    return x[k:e]
t={c:sum(db(D.get(a,'')).count(chr(10)) for a in re.findall(r\"'([^']+)'\",l)) for c,l in S.items()}
[print('  %-10s %4d'%(c,v)) for c,v in sorted(t.items(),key=lambda x:-x[1])]
"
```

원칙: **공용은 저수준 부품까지만.** `arc()`/`lineTo()` 급 프리미티브(날 한 자루,
불티 하나)는 공유해도 되지만, "이펙트 전체"를 공유하면 캐릭터가 서로 구분되지 않는다.
연출 언어는 컨셉에서 뽑아낸다 — 검이면 참격 궤적, 총이면 총구 화염·예광·탄피,
사쿠라면 부채꼴 워시 + 손으로 그린 꽃잎.

부수 효과로 얻어갈 것: 그 캐릭의 **핵심 상태를 화면에 보이게** 하면 개성이자 정보가 된다
(알바 = 충전 링, 플린타 = 실린더 잔탄 6칸).

### 1-5c. 눈 부담 — 풀스크린 섬광 금지

`drawComboFlash` 는 이미 **가장자리 비네팅 + 알파 0.30 상한**으로 바뀌어 있다
(주석에 "밤에 눈 부담" 이유가 적혀 있다). 새 능력에서 화면 전체를 흰색으로
덮지 말 것. 넓은 범위를 표현해야 하면 **띠·링·기둥** 처럼 형태를 주면 된다
(예: `firstLight` = 지평선 가로 띠).
`state.comboFlash` 값의 기존 범위는 20~60 이다. 새 값은 이 안에서 고른다.

**설명 문구(`desc`)와 실제 연출을 일치시킬 것.** "화면 전체 섬광"이라 써두면
읽는 사람이 눈 아픈 걸 예상한다 — 실제로 그러지 않는데도.
...
**연출은 그 캐릭의 물건이 형체로 보여야 한다.** 알바 초안이 "세로 막대 + 눈금 14칸이
차오르는" 형태였는데, 그건 게이지지 검이 아니고 하필 플린타 실린더(칸이 차오르는 통)와
구조가 같아서 함장에게 바로 지적당했다.

- ✗ 추상 도형에 기대기 — 막대·눈금·칸·원. 캐릭터가 달라도 다 똑같아 보인다.
- ✓ 실루엣을 그린다 — 대검이면 **양날 + 홈(fuller) + 십자 가드 + 폼멜**까지 그려야
  "검"으로 읽힌다. 그 위에 담금질 빛이 자루→칼끝으로 훑고 지나가는 식으로 시간을 태운다.

충전·장전 같은 "차오름"을 표현할 때 특히 게이지로 흐르기 쉽다. 차오르는 **대상**을
캐릭터의 물건으로 두면 해결된다(칼날이 달아오른다 / 실린더 칸에 불이 붙는다).
