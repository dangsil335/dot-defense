...
### 1-4. `CHAR_SIG_ABILITIES` — 시그니처 능력 체인
```
grep -n "const CHAR_SIG_ABILITIES" index.html
```
```js
carlotta: ['<시작능력>', '<조합>', '<4★>', '<8★>'],
```
...
```
캐릭터 전용 고대 유물. 두 곳 **모두** 넣어야 하고, `RELICS` 에 유물 정의도 필요하다.
...
⚠ 이 스캔은 **배열로 된 곳은 못 잡는다**(`GACHA_POOL`·`SR_TO_SSR`·`ANCIENT_RELIC_IDS`). 그건 §1-2·1-3·3-2 로 따로 챙긴다.
